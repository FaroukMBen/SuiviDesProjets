...
export default function ProjectMilestonesPage() {
    ...
    const handleLinkCampaign = async (campaignId: string) => {
        if (!await confirm({ title: "Liaison Campagne", message: "Voulez-vous lier ce projet à cette campagne ? Cela ne pourra pas être annulé facilement.", type: "warning" })) return;
        setIsLinking(true);
        try {
            await api.put(`/api/projects/${projectId}/link-campaign`, { campaignId });
            await fetchProjectAndMilestones();
            showToast("Projet lié à la campagne avec succès", "success");
        } catch (err) {
            console.error(err);
