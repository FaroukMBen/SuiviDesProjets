  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [tasksRes, ganttRes, milestonesRes] = await Promise.all([
        ...
        api.get(`/api/gantt-tasks/project/${projectId}`),
        campaignId ? api.get(`/api/milestones/campaign/${campaignId}`) : Promise.resolve({ data: { milestones: [] } })
      ]);

      const allItems: Item[] = [];

      tasksRes.data.tasks.forEach((t: any) => {
        allItems.push({
          id: t._id,
          type: 'kanban',
          title: t.title,
      ...
      ganttRes.data.tasks.forEach((t: any) => {
        allItems.push({
          id: t._id,
          type: 'gantt',
          title: t.title,
      ...
      milestonesRes.data.milestones.forEach((m: any) => {
        allItems.push({
          id: m._id,
          type: 'milestone',
          title: m.title,
    ...
    return (
      <div className={`grid grid-cols-7 border-t border-l ${isModern ? 'border-gray-100' : 'border-gray-200'} rounded-3xl overflow-hidden shadow-2xl shadow-gray-200/50 bg-white`}>
        {calendarDays.map((day, i) => {
          ...
          let dayItems = filteredItems.filter(item => {
              ...
              <div className="space-y-1 overflow-hidden">
                {displayItems.map((item, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className={`flex items-center gap-1.5 px-1.5 py-0.5 rounded border text-[9px] font-bold truncate shadow-sm transition-transform ${colorClass}`}
                      title={item.title}
                    >
              ...
              <div>
                ...
                <div className="grid gap-2">
                  {dayMilestones.map(m => {
                    ...
                    return (
                      <div key={m.id} className={`flex items-center gap-3 p-3 rounded-xl border ${colorClass}`}>
                    ...
                    return (
                      <div key={gantt.id} className="border border-indigo-100 rounded-2xl p-4 bg-indigo-50/40 relative overflow-hidden">
                        ...
                        <div className="flex items-start justify-between gap-4 mb-3">
                          ...
                          <Link href={`/projects/${projectId}/gestion/gantt`} className="shrink-0 flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest text-indigo-500 hover:text-indigo-700 bg-white/50 hover:bg-white px-2.5 py-1.5 rounded-lg border border-indigo-100 shadow-sm transition-all hover:scale-105">
                        ...
                        {subTasks.length > 0 ? (
                          <div className="ml-5 pl-4 border-l-2 border-indigo-200/60 space-y-2">
                            {subTasks.map(sub => (
                              <Link href={`/projects/${projectId}/gestion/kanban`} key={sub.id} className="flex items-center justify-between bg-white px-3 py-2 rounded-lg border border-indigo-50 shadow-sm transition-all hover:border-indigo-200 group cursor-pointer hover:shadow-md hover:scale-[1.01]">
              ...
              <div>
                ...
                <div className="grid gap-2">
                  {independentDayKanbans.map(k => {
                    ...
                    return (
                      <Link href={`/projects/${projectId}/gestion/kanban`} key={k.id} className={`flex items-center justify-between p-3 rounded-xl border opacity-90 transition-all hover:opacity-100 hover:shadow-md hover:scale-[1.01] ${colorClass}`}>
                        <div className="flex items-center gap-3">
